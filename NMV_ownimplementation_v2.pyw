...
def iteration(x_in):
    ...
    # sort
    for i in [0, 1, 2, 3]:
        y = [f(x_in[0]), f(x_in[1]), f(x_in[2]), f(x_in[3])]  # calculate function values
        y.sort()  # sort f-values ascending
        for j in [0, 1, 2, 3]:
            if f(x_in[i]) == y[j]:
                x_in[i], x_in[j] = x_in[j], x_in[i]  # swap values according to function values

    # centroid
    for i in [0, 1, 2]:
        m[0] += x_in[i][0]
        m[1] += x_in[i][1]
    m[0] = 1 / (len(x_in) - 1) * m[0]
    m[1] = 1 / (len(x_in) - 1) * m[1]

    r[0] = m[0] + alpha * (m[0] - x_in[3][0])  # reflexion
    r[1] = m[1] + alpha * (m[1] - x_in[3][1])
    if f(r) < f(x_in[0]):
        e[0] = m[0] + gamma * (m[0] - x_in[3][0])  # expansion
        e[1] = m[1] + gamma * (m[1] - x_in[3][1])
        if f(e) < f(r):
            x_in[3] = e
        else:
            x_in[3] = r
        ...
        if f(r) < f(x_in[2]):
            ...
            c[0] = m[0] + beta * (m[0] - x_in[3][0])  # contraction
            c[1] = m[1] + beta * (m[1] - x_in[3][1])
            if f(c) < f(x_in[3]):
                x_in[3] = c
            else:
                for i in [1, 2, 3]:
                    x_in[i][0] = x_in[0][0] + delta * (x_in[i][0] - x_in[0][0])  # Shrink
                    x_in[i][1] = x_in[0][1] + delta * (x_in[i][1] - x_in[0][1])
...
def main():
    ...
    with open('/Users/fuerstchristian/PycharmProjects/TestProject/Downhill-Simplex/nelder_mead.csv', 'w', newline="") as cs:
        write = csv.writer(cs)
    ...
    while 1:
        ...
        iteration(x)  # iteration of the algorithm
        ...
        with open('/Users/fuerstchristian/PycharmProjects/TestProject/Downhill-Simplex/nelder_mead.csv', 'a', newline="") as cs:
            write = csv.writer(cs)
            write.writerow(row1)
            write.writerow(row2)
            write.writerow(row3)
