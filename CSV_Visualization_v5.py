...
fig.add_trace(go.Surface(x=x, y=y, z=z, opacity=0.2, cmax=1, cmin=0, showscale=False))
...
fig.update_layout(title=dict(text='Nelder Mead Algorithm - Function ' + str(func[-1]) + '\t\t\t\t\t\t Optimal values ≈ (' +
                            str(round(x1[-1],4)) + ', ' + str(round(x2[-1],4)) + ')'
                            ))
