...
def f(v):
    if selection == "Himmelblau":
        z = math.pow(math.pow(v[0], 2) + v[1] - 11, 2) + math.pow(v[0] + math.pow(v[1], 2) - 7, 2)  #Himmelblau-Function
    else:
...
def main():
    ...
    with open('nelder_mead.csv', 'w', newline="") as cs:
        write = csv.writer(cs)
    ...
    while i < 20:
        ...
        with open('nelder_mead.csv', 'a',newline="") as cs:
            write = csv.writer(cs)
            write.writerow(row1)
            write.writerow(row2)
            write.writerow(row3)
