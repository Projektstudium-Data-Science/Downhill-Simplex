...
canvas1.create_window(342, 80, window=label_explanation_1)
...
canvas1.create_window(213, 105, window=label_explanation_2)

# Label Function
label_function = Label(root, text='Function:')
label_function.config(font=('Arial', 10), bg='lavender')
canvas1.create_window(67, 170, window=label_function)

# Function Dropdown - Menu
function_Menu = ttk.Combobox(root, textvariable=selection, values=['Himmelblau', 'Rosenbrock'])
function_Menu.current(0)
canvas1.create_window(174, 170, window=function_Menu)

# Label x1 / x2
label_x1 = Label(root, text='x1 - value:')
label_x1.config(font=('Arial', 10), bg='lavender')
canvas1.create_window(420, 170, window=label_x1)

label_x2 = Label(root, text='x2 - value:')
label_x2.config(font=('Arial', 10), bg='lavender')
canvas1.create_window(420, 200, window=label_x2)

# Entry x1 / x2
entry_x1 = Entry(root, textvariable='x1_Wert')
entry_x1.config(bg='lavender')
canvas1.create_window(523, 170, window=entry_x1)

entry_x2 = Entry(root, textvariable='x2_Wert')
entry_x2.config(bg='lavender')
canvas1.create_window(523, 200, window=entry_x2)

# optimize button
button_optimize = Button(root, font=('Arial', 13), command=optimize, text='Optimize!', bg='lightcoral')
canvas1.create_window(375, 260, window=button_optimize)
...
def f(v, selection):
    if (selection == 'Himmelblau'):
        ...
        z = math.pow(1 - v[0], 2) + 100 * math.pow((v[1] - math.pow(v[0], 2)), 2)                    #Rosenbrock
    return z
...
def sort(x):
    ...
    for i in [0, 1, 2, 3]:
        y = [f(x[0], selection), f(x[1], selection), f(x[2], selection), f(x[3], selection)]
        y.sort()
        for j in [0, 1, 2, 3]:
            if f(x[i], selection) == y[j]:
...
def iteration(m, x):
    ...
    if f(r, selection) < f(x[0], selection):
        e[0] = m[0] + beta * (m[0] - x[3][0])   # expansion
        e[1] = m[1] + beta * (m[1] - x[3][1])
        if f(e, selection) < f(r, selection):
            x[3] = e
            print("Expansion: ", e)
        else:
            x[3] = r
        ...
        if f(r, selection) < f(x[2], selection):
            x[3] = r
            print("Reflexion: ", r)
        else:
            if f(x[3], selection) < f(r, selection):
                h[0] = x[3][0]
                h[1] = x[3][1]
            else:
                h[0] = r[0]
            ...
            if f(c, selection) < f(x[3], selection):
                x[3] = c
                print("Kontraktion: ", c)
            else:
                for i in [0, 1, 2, 3]:
...
def main():
    ...
    z = [f(x[0], selection), f(x[1], selection), f(x[2], selection), f(x[3], selection)]
    ...
    while i < 20:
        ...
        if i < 10:
            ...
            c = [f(x[0], selection), f(x[1], selection), f(x[2], selection), f(x[3], selection)]
        ...
        i += 1
    ax.scatter(x[0][0], x[0][1], f(x[0], selection), c='red')
